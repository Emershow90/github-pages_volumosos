 ...
 * 
 * ConsolidationPanelV2.tsx
 * 
 * Painel visual para gerenciar registros consolidados por data.
 * Suporta uso com ou sem props (fallbacks automáticos das stores globais).
 */

import React, { useState, useMemo } from "react";
import { ConsolidationService } from "../services/consolidationService.v2";
...
import { googleSheetsService as defaultGoogleSheetsService } from "../services/googleSheetsService";
...
export const ConsolidationPanel: React.FC<ConsolidationPanelProps> = ({
  historico: propsHistorico,
  setores: propsSetores,
  colaboradores: propsColaboradores,
  currentRole: propsCurrentRole,
  spreadsheetId: propsSpreadsheetId,
  googleSheetsService: propsGoogleSheetsService,
  addToast: propsAddToast,
}) => {
  // Fallbacks de stores quando não passados via props
  const storeHistorico = useHistoryStore((s) => s.historico);
  const storeSetores = useSectorStore((s) => s.setores);
  const storeColaboradores = useCollaboratorStore((s) => s.colaboradores);
  const storeCurrentUser = useUserStore((s) => s.currentUser);
  const storeAddToast = useNotificationStore((s) => s.addToast);

  const historico = propsHistorico ?? storeHistorico ?? [];
  const setores = propsSetores ?? storeSetores ?? [];
  const colaboradores = propsColaboradores ?? storeColaboradores ?? [];
  const spreadsheetId = propsSpreadsheetId ?? "CONFIGURED_SPREADSHEET";
  const googleSheetsService = propsGoogleSheetsService ?? defaultGoogleSheetsService;
  const addToast = propsAddToast ?? storeAddToast;
    ...
    <div className="space-y-5 p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <button
          ...
          disabled={loading}
          className="rounded-xl bg-zinc-900/80 border border-zinc-800 p-3.5 text-left hover:border-indigo-500/40 hover:bg-zinc-800/50 transition-all disabled:opacity-50 group cursor-pointer"
        >
        ...
        <button
          ...
          disabled={loading}
          className="rounded-xl bg-zinc-900/80 border border-zinc-800 p-3.5 text-left hover:border-emerald-500/40 hover:bg-zinc-800/50 transition-all disabled:opacity-50 group cursor-pointer"
        >
        ...
        <button
          ...
          className="rounded-xl bg-zinc-900/80 border border-zinc-800 p-3.5 text-left hover:border-amber-500/40 hover:bg-zinc-800/50 transition-all disabled:opacity-50 group cursor-pointer"
        >
        ...
        <button
          ...
          className="rounded-xl bg-zinc-900/80 border border-zinc-800 p-3.5 text-left hover:border-zinc-600 hover:bg-zinc-800/50 transition-all group cursor-pointer"
        >
...
export const ConsolidationPanelV2 = ConsolidationPanel;
