...
class RealtimeSyncService {
  ...
  /**
   * Escuta em tempo real liderança / coordenador.
   */
  public startListeningLideranca(onUpdate?: (nome: string) => void) {
    const key = 'lideranca_live';
    if (this.authObservers.has(key)) return;
    ...
    const unsubscribeAuth = SupabaseService.onAuthStateResolved((state) => {
      ...
      SupabaseService.fetchTable<{ id?: string; nome?: string }>('lideranca')
        .then((rows) => {
          if (cancelled) return;
          if (rows && rows.length > 0 && rows[0].nome) {
            onUpdate?.(rows[0].nome);
          }
          if (isStaticBuild || !supabase) return;

          channel = supabase.channel(key)
            .on('postgres_changes', { event: '*', schema: 'public', table: 'lideranca' }, async () => {
              const fresh = await SupabaseService.fetchTable<{ id?: string; nome?: string }>('lideranca');
              if (fresh.length > 0 && fresh[0].nome) {
                onUpdate?.(fresh[0].nome);
              }
            })
            .subscribe();
        ...
        .catch((err) => console.error("[RealtimeSyncService] Erro lideranca:", err));
